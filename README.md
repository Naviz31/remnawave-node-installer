...
Перед запуском рекомендуется указать IP-адрес панели. Он не запрашивается интерактивно, чтобы установка действительно требовала только домен и ключ:

```bash
curl -fsSL https://raw.githubusercontent.com/Naviz31/remnawave-node-installer/main/install.sh | sudo env PANEL_IPS="203.0.113.10" bash
```

Если IP панели не указан, порт Node API останется закрытым firewall. Установку можно завершить, а затем добавить адрес и выполнить `repair`:
...
|---|---:|---|
| `PANEL_IPS` | пусто | один или несколько IP панели через запятую |
