...
<body><main><nav><a href="/" aria-label="{brand}">{brand}</a>{links}</nav><section class="hero"><span class="eyebrow">{brand}</span><h1>{title}</h1><p class="lead">{message}</p>{extra}</section><footer>Independent infrastructure services · {2026}</footer></main></body></html>'''
...
def generate_site(root: Path = SITE_ROOT, identity: str = "default") -> None:
    ...
    for relative, values in pages.items():
        ...
        target.write_text(_html(*values, brand=brand), encoding="utf-8")
